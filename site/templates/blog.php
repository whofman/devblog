  ...
  <main role="main">
    ...
    <?php if(param('tag')): 						// show tag results ?>
		<?php $tag = urldecode(param('tag'));
		      ...
		      echo '<h1 class="result">Posts met tag “<mark>' , $tag , '</mark>”</h1>';
		?>
    ...
    <?php else: // show latest articles ?>
	    ...
	    <?php $articles = $pages->find('blog')->children()->visible()->sortBy('date', 'desc')->paginate(3) ?>

	    <?php foreach($articles as $article): // article overview ?>
		<?php if ($article->draft() == 'yes') continue ?>
	    ...
	    <article>
	      ...
	      <a href="<?php echo $article->url() ?>">[meer →]</a></p>
	    </article>
